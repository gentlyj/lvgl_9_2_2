...
//   printf("text length: %d\n", txt_length);
  ...
  while (j < txt_length) {
    uint32_t c;
    c = lv_text_encoded_next(txt, &i);
    ch_enc[j++] = c; // lv_text_encoded_next(txt, &i);
  }
  ...
  while (i < txt_length) {
    ...
    } else if (lv_text_is_arabic_vowel(
                   ch_enc[i + 1])) { // Next character is a vowel
      if (lv_text_is_arabic_vowel(ch_enc[i + 2])) {
        idx_next = lv_ap_get_char_index(
            ch_enc[i + 3]); // Skip the vowel character to join with the
                            // character after it
      } else {
        idx_next = lv_ap_get_char_index(
            ch_enc[i + 2]); // Skip the vowel character to join with the
                            // character after it
      }
    }
    ...
    uint8_t conjunction_to_previous =
    ...
    if (lam_alef) {
      ...
      i += 2;
      // 特殊情况， 如果是 لَا
      // 这种把 0x644, 0x627 合并为 0xfefb لا
      // 如果中间是0x644, 0x627 中间还有一个 0x64e 的注音
      // 就直接再多跳一个字符， lvgl 之前没考虑中间还有注音的情况
      // 但是这么处理也不不完美， 会丢掉一个注音 
      if (ch_enc[i - 1] == 0x064e) {
        // 打开注释也不太对
        //  注意本来是标在 0x644 头上， 打开注释就会标在 0x627 头上了
        //   j++;
        //   ch_fin[j] = 0x64e;
        i++;
      }
    ...
    if (conjunction_to_previous && conjunction_to_next)
      ...
      ch_fin[j] = ap_chars_map[index_current].char_end_form +
                  ap_chars_map[index_current].char_beginning_form_offset;
    else if (conjunction_to_previous && !conjunction_to_next)
      ch_fin[j] = ap_chars_map[index_current].char_end_form;
    else
      ch_fin[j] = ap_chars_map[index_current].char_end_form +
    ...
    if (c == (ap_chars_map[i].char_offset + LV_AP_ALPHABET_BASE_CODE))
      return i;
             ...
             || c == (ap_chars_map[i].char_end_form +
                      ap_chars_map[i]
                          .char_beginning_form_offset) // is it a Beginning form
             || c == (ap_chars_map[i].char_end_form +
                      ap_chars_map[i]
